...
async def raw_cmsnt_products(db: Session = Depends(get_db), user: User = Depends(verify_api_key)):
    ...
    # Debug logging
    print(f"[DEBUG API PRODUCTS] Returning {len(result)} products as FLAT ARRAY. First: {result[0] if result else 'None'}")
    
    # Return flat array directly for maximum compatibility with ShopCloneV7
    return result
